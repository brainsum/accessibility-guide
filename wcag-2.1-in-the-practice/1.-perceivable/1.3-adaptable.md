...
The structure and content of the page must be structured in such a way that it can be displayed in different ways, e.g. in spoken or simplified form.

If the content and structural information are displayed and linked in a way that is intelligible to the software, they can display it to them according to the user's abilities. If it is not possible to cover it with algorithms, only the information that is absolutely necessary for the user should be displayed and it cannot be deviated from!
...
Identify parts of the page with ARIA landmarks.

Screen readers can use to jump between of them.

{% hint style="warning" %}
<mark style="color:orange;">No items other than those with ARIA landmarks can be included! That is, the direct children of the element cannot be elements that do not have a top-level role attribute (HTML5 elements have a default):</mark> <mark style="color:orange;"></mark><mark style="color:orange;">`banner`</mark><mark style="color:orange;">,</mark> <mark style="color:orange;"></mark><mark style="color:orange;">`main`</mark><mark style="color:orange;">,</mark> <mark style="color:orange;"></mark><mark style="color:orange;">`complementary`</mark><mark style="color:orange;">,</mark> <mark style="color:orange;"></mark><mark style="color:orange;">`contentinfo`</mark><mark style="color:orange;">.</mark>
{% endhint %}
...
3. Optionally\* each area individually with `aria-label`, `title` inline, or `aria-labelledby` if there is an unspecified heading (`h1` - `h6`) element.

\*of more than one exists of the same type on the page we must label them, but region role must have a label!
...
For example, we would need h7 or when we don’t have an access to the DOM, we can add this role via JS.
...
{% hint style="warning" %}
If you use more than one of the same type of landmarks on the page, each landmark must have an title/name, except for <mark style="color:red;">`region`</mark> <mark style="color:red;"></mark><mark style="color:red;">role where it’s a must have!</mark>
{% endhint %}

If the first element of the landmark is a heading, it can be used as the title of the landmark by referencing it with `aria-labelledby` (:warning:no need to put a `#` in front of it).
...
Use the group and radiogroup roles to group the logically related elements.
...
`region` role to identify each area of the page.
...
Use as many semantic elements as possible that were also mentioned in the html5 slide.
...
* Quote should have a `<q>` or `<blockquote>` and, if you know, a `<cite>` **** element to indicate the source_._
...
{% embed url="https://www.w3.org/TR/WCAG21/#input-purposes" %}
